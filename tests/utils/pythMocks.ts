...
import { Magic } from "@pythnetwork/client";
import { Keypair, PublicKey } from "@solana/web3.js";
import { Oracles, createMockAccount, storeMockAccount } from "./mock";
 ...
 * @returns
 */
export const createPriceAccount = async (
  wallet: Wallet,
  newAccount: Keypair = Keypair.generate()
) => {
  return createMockAccount(mockProgram, PYTH_ACCOUNT_SIZE, wallet, newAccount);
};
 ...
 * @param usdcDecimals
 * @param verbose
 * @param keypairs - overrides keypairs used for the oracles
...
export const setupPythOracles = async (
  ...
  let wsolPythOracle = await createPriceAccount(wallet, keypairs.wsol);
  ...
  let usdcPythOracle = await createPriceAccount(wallet, keypairs.usdc);
  ...
  let tokenAPythOracle = await createPriceAccount(wallet, keypairs.a);
  ...
  let tokenBPythOracle = await createPriceAccount(wallet, keypairs.b);
